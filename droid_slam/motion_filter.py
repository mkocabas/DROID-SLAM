...
class MotionFilter:
    ...
    def track(self, tstamp, image, depth=None, intrinsics=None, mask=None):
        ...
        if self.video.counter.value == 0:
            ...
            self.video.append(tstamp, image[0], Id, 1.0, depth, intrinsics / 8.0, gmap, net[0,0], inp[0,0], mask[0])
        ...
        ### only add new frame if there is enough motion ###
            ...
            if delta.norm(dim=-1).mean().item() > self.thresh:
                ...
                net, inp = self.__context_encoder(inputs[:,[0]])
                self.net, self.inp, self.fmap = net, inp, gmap
                self.video.append(tstamp, image[0], None, None, depth, intrinsics / 8.0, gmap, net[0], inp[0], mask[0])
